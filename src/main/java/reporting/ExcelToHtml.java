...
public class ExcelToHtml {
	...
	private final List<Integer> skipList = new ArrayList<Integer>();

	public static final class SheetCellSpan {
		// public int sheetIdx;
		private int sheetStartColumn;
		private int sheetEndColumn;

		public SheetCellSpan(int start, int end) {
			setSheetStartColumn(start);
			setSheetEndColumn(end);
		}

		public int getSheetStartColumn() {
			return sheetStartColumn;
		}

		public void setSheetStartColumn(int sheetStartColumn) {
			this.sheetStartColumn = sheetStartColumn;
		}

		public int getSheetEndColumn() {
			return sheetEndColumn;
		}

		public void setSheetEndColumn(int sheetEndColumn) {
			this.sheetEndColumn = sheetEndColumn;
		}
	}

	private final LinkedList<SheetCellSpan> sheetCellSpanList = new LinkedList<SheetCellSpan>();
	...
	public void addSheetCellSpan(int sheetIdx, SheetCellSpan columnSpan) {
		sheetCellSpanList.add(sheetIdx, columnSpan);
	}

	public static SheetCellSpan createSheetCellSpan(int start, int end) {
		return new SheetCellSpan(start, end);
	}

	public void skipSheet(int sheetIdx) {
		skipList.add(sheetIdx);
	}
	...
	public void printStyles() {
		...
		try {
			...
			// InputStreamReader(getClass().getResourceAsStream(new File(".",
			// "//TestData//excelStyle.css").getAbsolutePath())));
	...
	private void printSheets() {
		...
		for (Integer idx = 0; idx < noOfSheets; idx++) {
			if (skipList.contains(idx))
				continue;
			if (idx < sheetCellSpanList.size() && sheetCellSpanList.get(idx) != null) {
				this.firstColumn = sheetCellSpanList.get(idx).getSheetStartColumn();
				this.endColumn = sheetCellSpanList.get(idx).getSheetEndColumn();
				this.gotBounds = true;
				printSheet(idx);
	...
	public void printSheet(int sheetNo) {
		printSheet(wb.getSheetAt(sheetNo));
	}
