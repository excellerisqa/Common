...
public class LARS_FTPUpload {
	...
	public static boolean generateDatFilesFTP(String reportNameWithExtn, String newFileName, String destination, String aAccession, String aSeqNo) {
		boolean result = true;
		if (CommonFunctions.copyFileNewName("\\TestData\\FTP\\", reportNameWithExtn, "\\TestData\\FTP\\New FTP\\", newFileName)) {
			System.out.println("replacement begins");
			File file1 = new File(".", "\\TestData\\FTP\\New FTP\\" + newFileName);
			...
			try {
				...
				newContent = oldContent.replaceAll(accession, aAccession);
				newContent = newContent.replaceAll(seqNo, aSeqNo);
				...
				fr.close();
				System.out.println("replacement finished");
				/* copy that file to \\sdwvapbzt505\BRLDataLoad\FileLn\ */
				// copyFile("\\TestData\\FTP\\New FTP\\", newFileName,destination);
	...
	public static String incrementAccessionNum(String currentAccession, String reportName) {
		...
		if (reportName.equals("Lifelabs"))
			...
			// newAccession = currentAccession.substring(0, 1) +
			// (Integer.parseInt(currentAccession.split("-18")[0].split("M")[1]) + 1);
			// newAccession = newAccession + "-18";
		} else if (reportName.equals("LARS"))
			newAccession = Long.toString(Long.parseLong(currentAccession) + 1);
	...
	public LARS_FTPUpload(String host, String username, String password) throws Exception {
	...
	public LARS_FTPUpload() throws Exception {
	...
	public void uploadFTPFile_bk(String localFileFullName, String fileName, String hostDir) throws Exception {
		try {
	...
	// Method to upload the File on the FTP Server
	public void uploadFTPFile(String fileName, String hostDir) throws Exception {
	...
	public static boolean copyFile(String sourceFileDirectory, String fileNameWithExtn, String newFileDirectory) {
		try {
			...
			File destFile = new File(newFileDirectory + "//" + fileNameWithExtn);
	...
	/*
	 * search invoice number from PDF files
	 */
	@SuppressWarnings("unused")
	public static boolean searchPDFInvoice(String aPDFFileName, String aInvoice) {
		boolean result = true;
		try {
			URL TestURL = new URL(aPDFFileName);
			PDDocument doc = null;
			BufferedInputStream file = null;
			String output = null;
			URL urlOfPdf = TestURL;
