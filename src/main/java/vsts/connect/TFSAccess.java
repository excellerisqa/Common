...
public class TFSAccess {
	...
	/**
	 * The URL to your Team Foundation Server, including virtual directory but no
	 * collection name (like "http://server:8080/tfs").
	 */
	...
	// "https://archivell-excelleris.visualstudio.com/";
	public static String COLLECTION_URL = "https://lifelabs.visualstudio.com/";

	/*
	 * Authentication information. HTTP_PROXY_URL should be set to null if none is
	 * desired.
	 */

	public static String USERNAME = "qateam"; // "";//$NON-NLS-1$ //qateam1
	public static String PASSWORD = "q3s5arp7jutnoih4xtmu7tfeo5ufa6cizlr2ob7dda32iygai4qq"; //$NON-NLS-1$
	...
	/**
	 * A team project name (without the leading "$/") where files, work items, and
	 * builds can be created and modified.
	 */
	...
	/**
	 * Set this to a writable UNC share (like "\\server\share\drops") where build
	 * results can be copied.
	 */
	public static String BUILD_DROP_LOCATION = ""; //$NON-NLS-1$

	/**
	 * Connect to TFS using a set of credentials that uses the currently logged user
	 * in case no user name was provided otherwise it uses the provided credentials
	 *
	 * @return The Team Project Collection connected to
	 */
	public TFSAccess(String projectname) {
	...
	public static TFSTeamProjectCollection connectToTFS() {
		...
		System.setProperty("com.microsoft.tfs.jni.native.base-directory", new File(".", "//native//").getAbsolutePath());
	...
	/**
	 * Checks if the build server version is older than TFS2010 and prints an error
	 * message
	 *
	 * @param buildServer
