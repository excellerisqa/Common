...
public class SnippetsSamplesConnectionAdvisor implements ConnectionAdvisor {
	...
	public SnippetsSamplesConnectionAdvisor(final URI httpProxyURI) {
	...
	private class SamplesHTTPClientFactory extends DefaultHTTPClientFactory {
		...
		/**
		 * Overrides configureClientProxy method to set the value of the http_proxy on
		 * the HTTP Connection.
		 */
		@Override
		public void configureClientProxy(final HttpClient httpClient, final HostConfiguration hostConfiguration,
			...
			log.info("Proxy URL set to " //$NON-NLS-1$
					+ httpProxyURI.getHost() + ":" //$NON-NLS-1$
					+ (httpProxyURI.getPort() == -1 ? 80 : httpProxyURI.getPort()));

			hostConfiguration.setProxy(httpProxyURI.getHost(),
					httpProxyURI.getPort() == -1 ? 80 : httpProxyURI.getPort());
		}
	}
	...
	static class SamplePersistenceStoreProvider extends DefaultPersistenceStoreProvider {
		...
		public SamplePersistenceStoreProvider() {
			...
			/*
			 * Build a {@link PersistenceStore} which maps to a subdirectory inside the
			 * user's home directory (more precisely, where the Java system property
			 * "user.home" points). See the Javadoc on {@link
			 * VersionedVendorFilesystemPersistenceStore} for how the vendor name,
			 * application name, and version are mixed into the path on each platform.
			 */

			final FilesystemPersistenceStore baseStore = new VersionedVendorFilesystemPersistenceStore(
		...
		/**
		 * Returns a {@link FilesystemPersistenceStore} for storing configuration data.
		 */
		@Override
	...
	public WebServiceFactory getWebServiceFactory(final ConnectionInstanceData instanceData) {
		return new DefaultWebServiceFactory(getLocale(instanceData), new DefaultTransportRequestHandler(instanceData,
